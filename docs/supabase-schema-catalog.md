...
| `slug` | `text` | `ProviderInfo.publicSlug` | Server-managed canonical provider slug, unique per vertical. |
...
Authenticated providers may insert and update ordinary setup, profile, contact,
branding, availability, language, and vertical fields under the existing owner
RLS policies. Column-level grants prevent them from directly inserting or
updating `id`, canonical `slug`, `custom_slug`, `plan_tier`, `created_at`, or
`updated_at`; `owner_user_id` is accepted only during insert. Canonical slugs are
generated by the database trigger during provider creation and remain unchanged
during ordinary profile edits. Future custom-slug changes must use a trusted
server endpoint that verifies entitlement before performing the service-role
write. The current `plan_tier` column is transitional metadata, not a complete
billing or entitlement boundary.
