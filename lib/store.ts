...
export function materializeVerticalServices(drafts: ServiceDraft[]): Service[] {
  return drafts.map((draft) => ({
    ...
    capacityScope: draft.capacityScope,
    maxPartySize: parseMaxSpots(draft.maxPartySize),
    cost: draft.cost,
...
export function createBlankServiceDraft(vertical?: VerticalId): ServiceDraft {
  return {
    ...
    endTime: "",
    maxSpots: "",
    // Restaurants sell each seating separately; everything else sells the date.
    capacityScope: vertical === "restaurant" ? "slot" : undefined,
    maxPartySize: "",
    cost: "",
...
export function normalizeServices(source?: Service[] | null): Service[] {
  return (source ?? []).map((service) => ({
    ...
    capacityScope: service.capacityScope,
    maxPartySize: parseMaxSpots(service.maxPartySize),
    cost: service.cost,
